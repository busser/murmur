...
// NewClientWithHTTPClient returns a Lambda Extensions API client with a custom HTTP client
func NewClientWithHTTPClient(awsLambdaRuntimeAPI string, httpClient HTTPClient) *Client {
	baseURL := fmt.Sprintf("http://%s/2020-01-01/extension", awsLambdaRuntimeAPI)
	return &Client{
		baseURL:    baseURL,
		httpClient: httpClient,
